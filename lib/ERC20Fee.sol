...
contract ERC20Fee is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function _transfer(
        ...
        if (from == address(this)) {
            require(fromBalance >= amount + _feeAmount, "ERC20: transfer amount exceeds balance");
        } else {
    ...
    function _withdrawFee(address to) internal {
        ...
        _beforeTokenTransfer(from, to, _feeAmount);

        _balances[from] -= _feeAmount;
        _balances[to] += _feeAmount;
        emit Transfer(from, to, _feeAmount);

        _afterTokenTransfer(from, to, _feeAmount);
        _feeAmount = 0;
    }
